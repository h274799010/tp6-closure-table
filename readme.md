...
```
 > 它监听了`onAfterInsert`,`onBeforeUpdate`,`onBeforeDelete`事件,如果你使用了 `create()`,或对实例使用了 `update(), restore()` 它将自动维护树结构.
...
Menu::isolated()->where('id', '>', 5)->select();
  
// 获取所有根
Menu::getRoots();
  
// 一个scope,同getRoots()
Menu::onlyRoot()->select();
```

* 以上`select...()`方法都包含一个query构造器,如`getDescendants()`对应有一个`queryDescendants()`,

  这使得你可以在查询中加入更多条件或 'order',
  
  你可以这样使用
  
  `$menu->queryDescendants()->where('id', '>', 5)->order('sort','desc')->get();`
  
  > 注意 `getRoot()`,`getParent()`,`getRoots()`,`getIsolated()`4个方法没有query构造器

* 如果你想获取只包含单个或多个列的结果可以在`select...()`方法里传入参数,如:
...
`ClosureTable` 监听了模型的 `BeforeDelete` 事件
...
```bash
$ composer require hsen/tp6-closure-table
```

- 你的模型中必要的列 `id`,`pid`,

- `closure` 表必要的列 `ancestor`,`descendant`,`depth`
...
1. 在`model`里引入 trait `hs\ClosureTable\Traits\ClosureTable`.
...
use think\Model;
use hs\ClosureTable\Traits\ClosureTable;
...
非常感谢composer包jiaxincui/closure-table
## License

(https://github.com/h274799010/tp6-closure-table/readme.md)
